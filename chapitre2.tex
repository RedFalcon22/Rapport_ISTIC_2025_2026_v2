...
BF-N07 & Gérer les réservations
        & Lorsqu'un visiteur souhaite planifier une démonstration, l'assistant collecte ses données et lui résèrve une démonstration de netethic\\
\hline
...
BF-S01 & Filtrer les résultats par catégorie
        & L'utilisateur choisit l'une des catégories de la plateforme avant de poser sa question, ce qui limite la recherche aux documents de ce thème et améliore la précision des réponses. \\
\hline
...
BF-S04 & Gestion de session
        & Chaque session utilisateur est conservée temporairement avec l'historique de conversation et les informations d'identité (nom, e-mail, téléphone). \\
\hline
...
BF-E06 & Mémoriser le profil utilisateur
        & L'assistant mémorise les informations révélées au fil des échanges (prénom, sources de stress, situation scolaire) dans un profil temporaire pour personnaliser les réponses lors des sessions suivantes. \\
\hline
\end{tabular}
\end{table}
...
\textbf{ID} & \textbf{Besoin} & \textbf{Description} \\
\hline
BF-C01 & Classifier les textes automatiquement
        & Le système analyse chaque texte soumis et détermine automatiquement la stratégie adaptée : LLM avec ancrage documentaire si des références juridiques pertinentes sont disponibles, LLM seul sinon. En cas d'échec, il bascule sur les heuristiques pour garantir une réponse. \\
\hline
...
\hline
BNF-02 & Sécurité
        & Les clés de connexion ne sont jamais exposées côté navigateur, les communications sensibles transitent via HTTPS, les requêtes malveillantes sont filtrées, et le débit des messages par utilisateur est limité afin de protéger le service contre les abus. \\
\hline
...
\hline
BNF-04 & Scalabilité
        & Tous les composants sont conteneurisés via Docker \cite{docker}, permettant le déploiement de plusieurs instances selon la charge. Le classificateur traite plusieurs textes simultanément grâce à Apache Spark. \\
\hline
...
BNF-06 & Conformité RGPD
        & Aucun contenu de message n'est conservé au-delà de la session. Les profils utilisateurs du module de soutien émotionnel sont stockés temporairement pour personnaliser les échanges, sans conservation permanente ni partage avec des tiers. \\
\hline
BNF-07 & Multilingue
        & L'interface et les réponses sont disponibles en français et en anglais, avec détection automatique de la langue de l'utilisateur. \\
\hline
BNF-08 & Robustesse
