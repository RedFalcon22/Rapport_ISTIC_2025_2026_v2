...
La Figure~\ref{fig:support_interface} montre l'interface principale du assistant technique.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{interface-png/acceuil-support-chatbot.png}
\caption{Interface principale du assistant technique}
\label{fig:support_interface}
\end{figure}

\subsection{Interface contact support}

Lorsque le assistant technique ne trouve pas de réponse dans la documentation,
...
\begin{table}[H]
\centering
\caption{Résultats des tests automatisés — assistant technique (35 tests, exécution du 2026-05-17)}
...
Le assistant émotionnel est intégré au assistant technique et s'active automatiquement
...
La Figure~\ref{fig:emotional_accueil} présente l'interface du assistant émotionnel.
...
\caption{assistant émotionnel — réponse empathique face à une tristesse modérée}
...
pour le assistant technique.
Le assistant émotionnel présente deux particularités : le risque LLM08 est
...
\begin{table}[H]
\centering
\caption{Scores RAGAS — assistant technique (évaluation partielle, 10~questions)}
...
Le assistant émotionnel n'utilise pas d'architecture RAG~;
...
et le assistant technique (Streamlit) via son port dédié.
