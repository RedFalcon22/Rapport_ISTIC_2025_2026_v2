...
le LLM conversationnel avec profil persistant pour l'assistant émotionnel,
...
\begin{itemize}
    ...
    \item \textbf{Base documentaire vectorielle (ChromaDB) :} Stocke les extraits de documents
          ...
          Chaque module possède sa propre collection : le module Noa assistant et l'assistant technique
    ...
    \item \textbf{Orchestrateur LangChain :} Coordonne les étapes de traitement
          pour les modules conversationnels (Noa assistant, assistant technique, assistant émotionnel) :
...
\begin{figure}[H]
\centering
\hspace*{-0.1\textwidth}\includegraphics[width=1.2\textwidth]{diagramme/13_deploiement_uml.png}
\caption{Vue d'ensemble du déploiement — 4 stacks indépendantes de la plateforme NetEthic AI}
\label{fig:deploiement}
\end{figure}

\subsection{Diagramme de déploiement — modules conversationnels}

Les trois modules conversationnels (Noa assistant, assistant technique et assistant émotionnel)
partagent une infrastructure Docker Compose similaire : un conteneur backend FastAPI,
un conteneur frontend, un conteneur Redis pour la gestion des sessions et de la mémoire,
et un accès commun au serveur LLM Mistral-7B sur kaisens.fr.
...
\begin{itemize}
  ...
  \item \textbf{assistant technique :} utilise LangChain avec un pipeline RAG de 10 étapes
        (classification d'intention, réécriture, cache cosinus, recherche LIVE/PDF, génération).
  ...
  \item \textbf{assistant émotionnel :} n'utilise \textbf{pas} ChromaDB — il ne s'appuie pas
        ...
        (TTL 30 jours, profil utilisateur persistant, historique, buffer, cache de classification).
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Module 1 — Noa assistant : Assistant virtuel du site public}

\subsection{Conception générale}

Noa assistant est l'assistant virtuel accessible sur le site public de Netethic.
...
La Figure~\ref{fig:composants_Noa assistant} illustre l'organisation interne du module Noa assistant en trois couches (présentation, métier et données) ainsi que les interactions entre ses composants principaux.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{diagramme/18_composants_noa.png}
\caption{Diagramme de composants — Module Noa assistant}
\label{fig:composants_Noa assistant}
\end{figure}

\subsection{Diagramme de séquences}

La Figure~\ref{fig:seq_Noa assistant} illustre le déroulement d'une interaction typique avec Noa assistant
pour une question factuelle.

\begin{figure}[H]
\centering
\hspace*{-0.1\textwidth}\includegraphics[width=1.2\textwidth]{diagramme/04_sequence_noa.png}
\caption{Diagramme de séquences — Module Noa assistant (question factuelle)}
\label{fig:seq_Noa assistant}
\end{figure}

\subsection{Diagramme d'état-transition}

La Figure~\ref{fig:etat_Noa assistant} représente les états possibles de Noa assistant
au cours d'une session de conversation.

\begin{figure}[H]
\centering
\hspace*{-0.1\textwidth}\includegraphics[width=1.2\textwidth]{diagramme/05_etat_noa.png}
\caption{Diagramme d'état-transition — Module Noa assistant}
\label{fig:etat_Noa assistant}
\end{figure}
...
Le classificateur repose sur une architecture distincte de celle des modules conversationnels :
il n'utilise pas de frontend interactif et fonctionne en mode batch.
...
Les modules Noa assistant et assistant technique reposent sur une architecture
...
\subsection{Module Noa assistant — sources documentaires publiques}

\paragraph{Extraction et conversion}\mbox{}\newline

La base de connaissances de Noa assistant est constituée à partir du contenu du site public
de Netethic.
...
Le Tableau~\ref{tab:kb_Noa assistant} présente les statistiques de la base de connaissances
constituée pour Noa assistant.

\vspace{6pt}
\begin{table}[H]
\centering
\caption{Statistiques de la base de connaissances — Module Noa assistant}
\label{tab:kb_Noa assistant}
\begin{tabular}{|l|c|}
\hline
...
\subsection{Modul'assistant technique — documentation interne}
...
en suivant le même processus d'enrichissement que pour la base de Noa assistant~:
...
La mise en place a établi les bases de connaissances RAG (177~segments pour Noa assistant,
