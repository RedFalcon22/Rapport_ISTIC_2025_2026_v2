...
\begin{enumerate}
    \item \textbf{Assistant virtuel public (Noa assistant) :} Un assistant conversationnel disponible sur le site de Netethic en français et en anglais, accessible à toute heure, sans intervention humaine.
    ...
    \item \textbf{assistant émotionnel :} Un système qui détecte les signaux de détresse dans les messages des utilisateurs et génère des réponses empathiques adaptées, en orientant l'utilisateur vers les ressources disponibles sur la plateforme.
...
Le projet suit un processus de développement structuré en plusieurs phases, appliqué à chacun des quatre modules :
...
\begin{enumerate}
    ...
    \item \textbf{Collecte et préparation :} Recueil des documents de référence propres au module (contenu du site Netethic, documentation interne, textes de loi français).
    \item \textbf{Conception :} Modélisation de l'architecture et des flux de traitement spécifiques au module.
    \item \textbf{Développement :} Implémentation du module à l'aide de cadres de développement spécialisés en intelligence artificielle.
    \item \textbf{Validation :} Tests de performance, évaluation de la qualité des réponses et retours utilisateurs pour chaque module.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Daily stand-ups (réunions quotidiennes) :}
          Chaque jour, nous restituons un état d'avancement journalier sur Microsoft Teams, indiquant la progression dans les tâches du backlog en cours.
    \item \textbf{Backlog hebdomadaire :}
          Chaque semaine, nous recevons un backlog contenant de nouvelles tâches à réaliser pour le sprint en cours.
\end{itemize}

La modélisation du système a été réalisée avec le langage UML (Unified Modeling Language — langage de modélisation unifié).
...
BF-S05 & Questions suggérées
        & Des questions prédéfinies sont proposées à l'utilisateur pour l'aider à explorer les fonctionnalités de la plateforme. \\
\hline
...
BF-E04 & Alerter en cas de crise
        & Si le message révèle des pensées suicidaires ou une intention de violence, l'assistant déclenche immédiatement une réponse d'urgence(urgences, soutien aux victimes, lutte contre les violences). \\
\hline
BF-E05 & Mémoriser le profil de l'étudiant
        & Au fil des échanges, l'assistant retient les informations partagées par l'étudiant (nom, prénom, sources de stress, situation scolaire, réussites) pour personnaliser ses réponses lors des sessions suivantes. \\
\hline
...
\subsection{Le personnel de la plateforme}
...
Mistral-7B \cite{mistral2023} via une API standard.
Un LLM (Large Language Model) est un modèle de langage entraîné sur de grandes quantités de texte pour générer et comprendre du langage naturel.

% ─────────────────────────────────────────────────────────────────────────────
\section{Diagramme de cas d'utilisation}

Le diagramme de cas d'utilisation modélise les interactions entre les acteurs et les fonctionnalités du système.
La Figure~\ref{fig:use_case} présente le système global : le visiteur interagit avec Noa assistant, l'utilisateur interne avec l'\textit{assistant technique} et l'assistant émotionnel, tandis que l'administrateur supervise l'ensemble et que le serveur LLM externe alimente tous les modules.
...
CU-01 & Poser une question (site public)
       & Visiteur
       & Le visiteur pose une question à propos du site en français ou en anglais. \\
\hline
...
CU-03 & Poser une question (support interne)
       & Personnel
       & L'utilisateur pose une question à propos de l'utilisation et la navigation sur la plateforme. \\
CU-03a & Sélectionner catégorie
        & Personnel
        & L'utilisateur pose une question de suivi sur le même sujet, et l'assistant réutilise les résultats de recherche précédents pour répondre de façon cohérente. \\
\hline
CU-03b & Consulter les suggestions de questions
        & Personnel
        & L'utilisateur choisit une des questions prédéfinies pour explorer les fonctionnalités. \\
\hline
CU-04 & Planifier un rendez-vous de support
       & Personnel
       & L'utilisateur demande à planifier un rendez-vous avec l'équipe technique.
         \\
\hline
CU-05 & Signaler un dysfonctionnement
       & Personnel
       & L'utilisateur signale un problème technique via un formulaire guidé. \\
\hline
CU-05b & Accéder au portail de support
        & Personnel
        & l'utilisateur accède directement au portail de support en ligne sans passer par le formulaire. \\
\hline
CU-06 & Bénéficier de l'assistant émotionnel
       & Etudiant
       & L'étudiant engage une conversation avec l'assistant émotionnel. \\
\hline
CU-06b & Choisir la langue de l'assistant émotionnel
        & Utilisateur interne
        &L'utilisateur sélectionne la langue de conversation (français ou anglais). \\
\hline
...
Ce chapitre a présenté le cadre général du projet : l'entreprise d'accueil,
la problématique traitée et la solution envisagée. Le chapitre suivant
expose les technologies et les choix architecturaux retenus.
