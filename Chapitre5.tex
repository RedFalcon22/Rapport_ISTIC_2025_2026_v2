...
automatisé (\texttt{run\_perf\_benchmark.py}), exécuté avec 3~répétitions par scénario
...
LLM01 & Injection de prompt
  & Classificateur LLM (température~0, déterministe) détecte et bloque les tentatives
    de \texttt{prompt\_injection} avant le pipeline principal ;
...
LLM09 & Désinformation
  & Escalade automatique vers des ressources d'urgence
    (n°~112, 0800~737~800, 3919) pour les catégories \texttt{suicidal}
    et \texttt{violence} ; le module évite tout conseil médical ou diagnostic. \\
\hline
LLM10 & Vol de modèle
...
\begin{itemize}
    ...
    \item \textbf{Adéquation linguistique :}
          ...
          (français, anglais ou arabe), sans code-switching non sollicité.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Génération des réponses~:}
          ...
          (\texttt{/start-session} puis \texttt{/chat}).
...
(juge~: \texttt{llama-3.3-70b-versatile} via Groq API).
La concision atteint un score parfait de~1,00, confirmant que la contrainte
de 3~phrases maximum imposée par le prompt système est systématiquement respectée.
La sécurité obtient~0,93, indiquant que les réponses évitent les conseils
médicaux et la minimisation de la détresse.

Un point d'attention ressort de l'analyse des scénarios~: le scénario arabe
(\texttt{ar-01}) a reçu une réponse en anglais (score langue~= 0,00),
...
L'évaluation est exécutée via le script \texttt{scripts/run\_eval.py}
sur le corpus \texttt{data/eval/scenarios.json} ;
les résultats détaillés par scénario sont sauvegardés dans
\texttt{data/eval/eval\_results.json}.
